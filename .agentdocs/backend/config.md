...
2. `config.LoadConfig` 负责设置默认值、读取文件、应用配置，并暴露给其它模块的全局变量（JWT、Redis、Postgres、限流、租户、TLS/ACME 等）。
...
6. 数据库 DSN、Redis 地址等敏感数据仅存于本地 `config.yaml`（已被 `.gitignore` 忽略），仓库仅提交 `config.yaml.example` 作为字段示例。
7. 文档及代码中面向用户的注释保持中文表达，必要的英文术语首次出现时附带中文说明。
