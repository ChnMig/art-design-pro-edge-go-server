...
2. `config.LoadConfig` 负责设置默认值、读取文件、应用配置，并暴露给其它模块的全局变量（JWT、Redis、Postgres、限流、租户等）。
...
5. 数据库 DSN、Redis 地址等敏感数据不得写入仓库示例值，统一放入 `config.yaml` 并通过部署脚本注入。
6. 文档及代码中面向用户的注释保持中文表达，必要的英文术语首次出现时附带中文说明。
