...
func GetTenantByCode(code string) (SystemTenant, error) {
	...
	err := postgres.GetClient().Where("code = ? AND status = 1", code).First(&tenant).Error
	if err != nil {
...
func GetTenant(tenant *SystemTenant) error {
	if err := postgres.GetClient().Where(tenant).First(tenant).Error; err != nil {
...
func AddTenant(tenant *SystemTenant) error {
	if err := postgres.GetClient().Create(tenant).Error; err != nil {
...
func UpdateTenant(tenant *SystemTenant) error {
	if err := postgres.GetClient().Updates(tenant).Error; err != nil {
...
func DeleteTenant(tenant *SystemTenant) error {
	if err := postgres.GetClient().Delete(tenant).Error; err != nil {
...
func FindAllTenants(tenants *[]SystemTenant) error {
	if err := postgres.GetClient().Find(tenants).Error; err != nil {
		zap.L().Error("failed to find all tenants", zap.Error(err))
		return err
	}
	return nil
