...
func GetDepartment(department *SystemDepartment) error {
	if err := postgres.GetClient().Where(department).First(department).Error; err != nil {
...
func AddDepartment(department *SystemDepartment) error {
	if err := postgres.GetClient().Create(&department).Error; err != nil {
...
func UpdateDepartment(department *SystemDepartment) error {
	if err := postgres.GetClient().Updates(&department).Error; err != nil {
...
func DeleteDepartment(department *SystemDepartment) error {
	if err := postgres.GetClient().Delete(&department).Error; err != nil {
...
func CountUsersByDepartmentID(departmentID uint, count *int64) error {
	if err := postgres.GetClient().Model(&SystemUser{}).Where("department_id = ?", departmentID).Count(count).Error; err != nil {
