...
func GetUserMenuData(userID uint) ([]SystemMenu, []SystemMenuAuth, error) {
	// 获取用户信息及其角色
	var user SystemUser
	if err := postgres.GetClient().Where(&SystemUser{Model: gorm.Model{ID: userID}}).First(&user).Error; err != nil {
		zap.L().Error("failed to get user", zap.Error(err))
		return nil, nil, err
	}
	// 获取该角色关联的所有菜单(包括权限)
	var role SystemRole
	if err := postgres.GetClient().Preload("SystemMenus").
...
func GetMenuData() ([]SystemMenu, []SystemMenuAuth, error) {
	...
	if err := postgres.GetClient().Find(&menus).Error; err != nil {
		zap.L().Error("failed to get menus", zap.Error(err))
		return nil, nil, err
	}
	var Auths []SystemMenuAuth
	if err := postgres.GetClient().Find(&Auths).Error; err != nil {
...
func GetMenuDataByRoleID(roleID uint) ([]SystemMenu, []SystemMenuAuth, []uint, []uint, error) {
	...
	if err := postgres.GetClient().Find(&allMenus).Error; err != nil {
		zap.L().Error("failed to get all menus", zap.Error(err))
		return nil, nil, nil, nil, err
	}
	// 获取所有权限
	var allAuths []SystemMenuAuth
	if err := postgres.GetClient().Find(&allAuths).Error; err != nil {
...
func AddMenu(menu *SystemMenu) error {
	if err := postgres.GetClient().Create(&menu).Error; err != nil {
...
func DeleteMenu(menu *SystemMenu) error {
	if err := postgres.GetClient().Delete(&menu).Error; err != nil {
...
func UpdateMenu(menu *SystemMenu) error {
	if err := postgres.GetClient().Omit("created_at").Save(menu).Error; err != nil {
...
func GetMenu(menu *SystemMenu) error {
	if err := postgres.GetClient().Where(menu).First(menu).Error; err != nil {
