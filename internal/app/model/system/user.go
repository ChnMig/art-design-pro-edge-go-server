...
import (
	...
	"api-server/internal/pkg/crypto"
)
...
func HashPassword(password string) (string, error) {
	return crypto.HashPasswordWithBcrypt(password)
}

// VerifyPassword 验证密码，新安装只支持bcrypt格式
func VerifyPassword(password, hashedPassword, passwordType string) bool {
	return crypto.VerifyBcryptPassword(password, hashedPassword)
}
...
func VerifyUser(tenantCode, account, password string) (SystemUser, SystemTenant, error) {
	...
	// 根据租户ID和账号查找用户
	err = postgres.GetClient().Where("tenant_id = ? AND account = ?", tenant.ID, account).First(&user).Error
	if err != nil {
...
func CreateLoginLog(log *SystemUserLoginLog) error {
	if err := postgres.GetClient().Create(log).Error; err != nil {
...
func AddUser(user *SystemUser) error {
	...
	if err := postgres.GetClient().Create(user).Error; err != nil {
...
func GetUser(user *SystemUser) error {
	if err := postgres.GetClient().Where(user).First(user).Error; err != nil {
		zap.L().Error("failed to get user", zap.Error(err))
		return err
	}
	return nil
...
func UpdateUser(user *SystemUser) error {
	...
	if err := postgres.GetClient().Updates(user).Error; err != nil {
...
func DeleteUser(user *SystemUser) error {
	if err := postgres.GetClient().Delete(user).Error; err != nil {
...
func FindAllUsers(users *[]SystemUser) error {
	if err := postgres.GetClient().Find(users).Error; err != nil {
