...
func UpdateRole(role *SystemRole) error {
	if err := postgres.GetClient().Updates(&role).Error; err != nil {
...
func AddRole(role *SystemRole) error {
	if err := postgres.GetClient().Create(&role).Error; err != nil {
...
func DeleteRole(role *SystemRole) error {
	if err := postgres.GetClient().Delete(&role).Error; err != nil {
...
func GetRole(role *SystemRole) error {
	if err := postgres.GetClient().Where(role).First(role).Error; err != nil {
...
func FindAllRoles(roles *[]SystemRole) error {
	if err := postgres.GetClient().Find(roles).Error; err != nil {
